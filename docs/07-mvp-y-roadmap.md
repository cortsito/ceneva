
## objetivo del mvp

validar que una ruta guiada mejora la continuidad de estudio y la comprensión frente a usar solo simulacros.

## mvp

- registro simple o modo invitado con progreso local.
- diagnóstico breve por área.
- una ruta navegable con las siete áreas.
- lecciones interactivas para una primera unidad por área.
- práctica por tema con feedback explicado.
- dashboard básico de progreso.
- enlace entre errores de práctica o simulacro y su tema.

## fuera del mvp

- chat con ia.
- gamificación, rachas y rankings.
- comunidad.
- planes de pago complejos.
- contenido de todas las unidades antes de validar la plantilla.
- recomendaciones predictivas avanzadas.

## secuencia recomendada

1. definir la guía fuente y el mapa completo de temas.
2. diseñar y validar la plantilla de una lección.
3. crear una vertical completa: un área, varias lecciones y práctica conectada.
4. implementar progreso y repaso.
5. integrar simulacros existentes.
6. expandir las demás áreas con el mismo sistema editorial.

## métricas de validación

- porcentaje de usuarios que completa una primera lección.
- porcentaje que vuelve a estudiar durante la semana.
- lecciones completadas por usuario activo.
- mejora de precisión antes y después de una lección.
- porcentaje de errores que terminan en una acción de estudio.
