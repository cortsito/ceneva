...
**la siguiente vertical en progreso es `cn-5-1-materia-y-sus-interacciones` de ciencias naturales, experimentales y tecnología.** conserva el patrón de tres tareas secuenciales en `docs/tasks/2026-09-16-task_etapa-5-cn-5-1-*.md`: cinco lecciones con ids reservados, un banco validado de veinticinco preguntas y su registro e integración visible como quinta entrada de `available_units`. se eligió esta unidad porque sus cinco temas y su único prerrequisito quedan dentro de la misma vertical; no expone contenido incompleto ni requiere una abstracción nueva. **la primera tarea (cinco lecciones) ya está completa**: `enlaces químicos`, `estados de agregación`, `conservación de la materia`, `conversión de temperatura` y `ley de coulomb` existen con sus cinco ids reservados cada una, `conservación de la materia` requiere `enlaces químicos` y las otras cuatro no tienen prerrequisito, y las lecciones de conversión de temperatura y ley de coulomb muestran fórmula, sustitución, aritmética y unidades paso a paso. la única muestra de la guía aplicable dentro de esta unidad (página 35, clasificación de nacl como enlace iónico) está referenciada en `cn-tipos-de-enlaces-01`; ninguna de las otras cuatro temas de `cn-5-1` tiene muestra aplicable en el bloque de la guía. las lecciones y reactivos de cálculo deben usar sustitución, unidades y explicaciones verificables, pero la aplicación reutiliza los límites genéricos existentes. diagnóstico, simulador, repaso global, guía de inicio y el resumen de `/progreso` permanecen exclusivamente en pensamiento matemático. después se repetirá el mismo patrón, por una primera unidad completa, para lengua y comunicación y ciencias sociales.
